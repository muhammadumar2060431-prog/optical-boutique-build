...
export function FaqSection() {
  ...
  const filteredFaqs = useMemo(() => {
    return publishedFaqs.filter((f) => {
      const matchesCategory =
        activeCategory === "All" || f.category?.trim() === activeCategory;
  ...
  return (
    <section className="bg-black py-20 sm:py-28 relative overflow-hidden border-b border-zinc-800 text-white">
      {/* Background subtle monochrome ambient glow */}
      <div
        className="pointer-events-none absolute -top-24 left-1/2 -translate-x-1/2 w-[600px] h-[300px] bg-white/[0.03] blur-[120px] rounded-full"
        aria-hidden="true"
      />

      <div className="mx-auto max-w-5xl px-4 sm:px-6 relative z-10">
        {/* Section Header (Black, White & Gray) */}
        <div className="text-center max-w-2xl mx-auto mb-12">
          <Reveal>
            ...
            <p className="mt-4 text-sm sm:text-base text-zinc-400 leading-relaxed">
              Everything you need to know about our prescription lenses, bespoke frame fitting, doorstep delivery, and optical guarantees.
            </p>
          </Reveal>
        </div>

        {/* Search & Category Tabs (Monochrome) */}
        <Reveal delay={100}>
          <div className="space-y-4 mb-10">
            ...
            <div className="relative max-w-md mx-auto">
              ...
              <Input
                ...
                className="pl-10 pr-4 py-2.5 h-11 bg-white border-white text-black font-semibold placeholder:text-zinc-500 placeholder:font-normal rounded-full shadow-lg shadow-black/40 focus-visible:ring-2 focus-visible:ring-black/30 focus-visible:border-white transition-all"
              />
            </div>

            {/* Category Filter Pills (Gray Background, White Text -> Hover: White Background, Black Text) */}
            {categories.length > 1 && (
                    ...
                    <button
                      ...
                      className={`px-4 py-1.5 rounded-full text-xs font-semibold transition-all duration-200 cursor-pointer border ${
                        isActive
                          ...
                          : "bg-[#666666] border-[#555555] text-white hover:bg-white hover:text-black hover:border-white hover:scale-105"
                      }`}
                    >
                      {cat}
                    </button>
        ...
        {/* FAQ Accordion List (Black, White, Gray) */}
        <Reveal delay={200}>
          {filteredFaqs.length === 0 ? (
            <div className="text-center py-12 px-4 rounded-2xl border border-dashed border-zinc-800 bg-zinc-950/60 max-w-xl mx-auto">
              <HelpCircle className="h-8 w-8 text-zinc-400 mx-auto mb-2 opacity-80" />
              <p className="text-sm font-medium text-white">No matching questions found</p>
              ...
              <Button
                ...
                className="mt-4 rounded-full text-xs border-zinc-700 bg-zinc-900 text-white hover:bg-zinc-800"
              >
                Clear filters
              </Button>
            </div>
                ...
                return (
                  <div
                    ...
                    className={`rounded-2xl border transition-all duration-300 px-5 sm:px-6 ${
                      isOpen
                        ? "border-white bg-white text-black shadow-2xl shadow-black/80"
                        : "border-[#555555] bg-[#666666] text-white hover:bg-[#5C5C5C]"
                    }`}
                  >
                    {/* Question Trigger */}
                    <button
                      ...
                      <div className="flex items-center gap-3.5 pr-4 min-w-0">
                        ...
                        <span
                          className={`text-sm sm:text-base font-semibold transition-colors duration-300 ${
                            isOpen ? "text-black" : "text-white drop-shadow-xs"
                          }`}
                        >
                          {faq.question}
                        </span>
